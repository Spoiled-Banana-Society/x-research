...
const LeaguesComponent = () => {
    ...
    // RTDB drafts/{leagueId}/numPlayers — often blocked by rules or wrong DB URL; API also returns numPlayers.
    const subscribeLivePlayerCounts = () => {
        ...
        const handler = (snapshot: import("firebase/database").DataSnapshot) => {
            ...
            const liveDrafts = Object.keys(data).map((key) => {
                const node = data[key]
                const n =
                    node && typeof node === "object" && typeof node.numPlayers === "number"
                        ? node.numPlayers
                        : 0
                return { id: key, numPlayers: n }
            })
            setNumOfPlayers(liveDrafts)
        }
        onValue(numPlayersRef, handler, (err) => {
            console.warn("[leagues] RTDB drafts/ listener failed (counts still work via API refresh):", err)
            setNumOfPlayers([])
        })
        return () => off(numPlayersRef, "value", handler)
    }
    ...
    // fetch leagues + RTDB subscription (cleanup on wallet change)
    useEffect(() => {
        if (!walletAddress) return
        Leagues.getLeagues(walletAddress).then((res) => setLeagueData(res))
        const unsub = subscribeLivePlayerCounts()
        return unsub
    }, [walletAddress])

    // While leagues are still filling (pre-draft), poll Firestore-backed counts so others joining updates the UI
    const fillingActiveCount =
        leagueData?.active?.filter((t) => t.roster?.RB === null).length ?? 0
    useEffect(() => {
        if (!walletAddress || fillingActiveCount === 0) return
        const id = setInterval(() => {
            Leagues.getLeagues(walletAddress).then((res) => setLeagueData(res))
        }, 4000)
        return () => clearInterval(id)
    }, [walletAddress, fillingActiveCount])
    ...
    useEffect(() => {
        ...
        if (leagueData?.active) {
            const live = numOfPlayers ?? []
            const data = leagueData.active.map((item) => {
                const found = live.find((draft) => draft.id === item._leagueId)
                const fromRtdb = found?.numPlayers
                const fromApi = typeof item.numPlayers === "number" ? item.numPlayers : 0
                // Prefer RTDB when we have a non-zero or explicit match; API is authoritative when RTDB missing/blocked
                const numPlayers =
                    fromRtdb !== undefined && (fromRtdb > 0 || live.some((d) => d.id === item._leagueId))
                        ? fromRtdb
                        : fromApi
                return { ...item, numPlayers: Math.max(fromApi, fromRtdb ?? 0) }
            })
            setMergedData(data)
        }
        ...
        useEffect(() => {
            if (walletAddress) {
                const fetchNFTs = async () => {
                    try {
                        const response = (await Settings.getNFTs(walletAddress)) as {
                            nfts: NFTProps[]
                        }
