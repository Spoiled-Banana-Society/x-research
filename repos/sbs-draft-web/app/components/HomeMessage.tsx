...
const LeaderBoardComponent: React.FC<Props> = (props) => {
    ...
    const [orderBy, setOrderBy] = useState<ORDERBY>(ORDERBY.SCORESEASON)
    const [showAll, setShowAll] = useState(false)
    const [view, setView] = useState<VIEW>(VIEW.LEAGUE)
    const [week, setWeek] = useState<string>(gameWeeks[0].gameWeek)
    const [level, setLevel] = useState<LEVEL>(LEVEL.HOF)
    ...
    const fetchAll = async (walletAddress: string, gameWeek: string, orderBy: string, level: string) => {
        const response = (await Leagues.allLeaderboard(walletAddress, gameWeek, orderBy, level)) as LeaderboardProps
    ...
    const fetchLeague = async (walletAddress: string, draftId: string, orderBy: string, level: string) => {
        const response = (await Leagues.leagueLeaderboard(walletAddress, draftId, orderBy, level)) as LeaderboardProps
        setOrderBy(ORDERBY.SCOREWEEK)
        setView(VIEW.LEAGUE)
        setLeaderboard(response)
    }
    ...
    const fetchLeagueInfo = async (walletAddress: string) => {
        const response = (await Leagues.getLeagues(walletAddress)) as { active: activeLeagueDataProps[] }
        setLeagues(response.active)
    }

    const fetchHofLeaderboard = async (walletAddress: string, orderBy: string, gameWeek: string) => {
        const response = (await Leagues.hofLeaderboard(walletAddress, ORDERBY.SCORESEASON, gameWeek)) as LeaderboardProps
    ...
    useEffect(() => {
        ...
        // FINALS
        homepage ? fetchLeague(walletAddress!, (leagueId ? leagueId : '2024-live-draft-finals'), orderBy, week) : fetchLeague(walletAddress!, leagueId!, orderBy, week)
            ...
            <div className="pt-16 text-center px-3 font-primary">
                <div>
                    ...
                    <h4 className="font-primary font-bold text-md lg:text-lg pb-12">
                        34.3 ETH {ethPrice ? `($${formatUSDVal(ethPrice * 34.3)})` : ''} Guaranteed Prize Pool <br />
    ...
    //                             <select className="text-black" onChange={(e) => selector(e.target.value)} defaultValue={level}>
