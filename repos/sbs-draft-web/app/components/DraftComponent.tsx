...
const DraftComponent: React.FC<DraftComponentProps> = (props) => {
    ...
    useEffect(() => {
        ...
        // sortBy + autoDraft: PlayerComponent loads via getDraftPreferences on join & after each pick
    }, [leagueId, walletAddress, dispatch])
    ...
    const searchChangeHandler = (payload: string) => {
        if (payload === "ADP") {
            Draft.updateDraftSortOrder(leagueId!, walletAddress!, "ADP").then(() => {
                dispatch(setDraftSort(SortState.ADP))
            }).catch((error) => {
                console.error("Error updating draft sort order:", error)
            })
            return
        }
        if (payload === "RANK") {
            Draft.updateDraftSortOrder(leagueId!, walletAddress!, "RANK").then(() => {
    ...
    const filterAndSortPlayers = (players: PlayerDataProps[], positions: string[]) => {
        ...
        if (expandInput) {
            ...
            let filteredAndSortedArray: any[] = []
    ...
    return (
        <div>
            ...
            {filteredAndSortedPlayers.map((player) => {
                return <DraftItemComponent key={player.ranking.rank} item={player} />
            })}
        </div>
    )
}
