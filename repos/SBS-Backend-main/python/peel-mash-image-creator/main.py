...
def download_blob(source_blob_name, final_file_name):
    ...
    try:
        ...
        bucket = storage_client.get_bucket('sbs-fantasy-prod-card-images-1')
...
def upload_blob(source_file_name, destination_blob_name):
    ...
    bucket_name = 'sbs-fantasy-prod-card-images-1'
...
def create_card(traits, token_id, isSpoiled, level, uuidFour):

    if isSpoiled:
        horizontal = 800
        start_vertical = 235
        offsets = [10,3,3,2,2,0,0,0,0,0,0,0,0,0]
        if level == 'Spoiled Pro':
            download_blob('spoiled-standard.png','file_import.png')
            sbs_image = Image.open('/tmp/file_import.png')
            font_color = (6,172,13)
        else:
            download_blob('spoiled-gold.png','file_import.png')
            sbs_image = Image.open('/tmp/file_import.png')
            font_color = (6,172,13)
    else:
        horizontal = 604
        ...
        if level == 'Pro':
            download_blob('standard.png','file_import.png')
            sbs_image = Image.open('/tmp/file_import.png')
            font_color = (213,178,82)
        else:
            download_blob('gold.png','file_import.png')
            sbs_image = Image.open('/tmp/file_import.png')
            font_color = (0, 0, 0)
...
def peelmash(request):
    ...
    if request_json and 'card' in request_json:
        ...
        if '_level' in card and card['_level'] in ["Spoiled Pro", "Spoiled Hall of Fame"]:
            isSpoiled = True


        traits = attribute_prep(card)
        uuidFour = str(uuid.uuid4().hex)
        create_card(traits, card['_cardId'], isSpoiled, card['_level'], uuidFour)
        card['_imageUrl'] = f'https://storage.googleapis.com/sbs-fantasy-prod-card-images-1/thumbnails/{card["_cardId"]}-{uuidFour}_350x490.png'
        return (card, 200)
    else:    
