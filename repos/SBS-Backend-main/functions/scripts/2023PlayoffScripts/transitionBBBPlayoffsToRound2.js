...
const findCardsMovingToRoundTwo = async () => {
    ...
    for(let i = 1; i <= 20; i++) {
        const leagueId = `BBB2023-round1-${i}`;
        const path = `${baseURL}/league/0x2f9c2123652cff3717fbd8edb1b256f16e9e4b80/drafts/${leagueId}/leaderboard/ScoreSeason/gameweek/2023REG-15`
...
const payoutElimatedCard = async (card) => {
    ...
    await db.createOrUpdateDocument(`draftTokenLeaderboard/2023REG-15/cards`, card.CardId, { Card: card }, true)
    await db.createOrUpdateDocument(`drafts/${card.LeagueId}/scores/2023REG-15/cards`, card.CardId, { Card: card }, true)
...
const createRankedPlayoffCardsList = async (cardsInPlayoffs) => {
    let leaderboard;
    try {
        let result = await db._db.collection(`draftTokenLeaderboard/2023REG-15/cards`).orderBy("ScoreWeek", "desc").get()
...
const create4PlayoffLeagues = () => {
    let leagueMap = {};
    const date = new Date('2023-12-25');
    const startdate = new Date('2023-12-19')
    for( let i = 0; i < 4; i++) {
        const leagueId = `BBB2023-round2-${i+1}`
        const league = {
            CurrentUsers: [],
            DisplayName: `BBB Round 2 Playoff League ${i+1}`,
            DraftType: "live",
            EndDate: date,
            IsLocked: true,
            LeagueId: leagueId,
...
const createRound2PlayoffLeagues = async (rankedPlayoffCards, leagueMap) => {
    const baseId = "BBB2023-round2-";
    let leagueNum = 1;
    ...
    for(let i = 0; i < rankedPlayoffCards.length; i++) {
        ...
        await db.createOrUpdateDocument(`drafts/${leagueId}/scores/2023REG-16/cards`, cardId, data, true)
        await db.createOrUpdateDocument(`draftTokenLeaderboard/2023REG-16/cards`, cardId, data, false)

        const week15Data = data
        for(let j = 4; j < 16; j++) {
            let gameweek;
            if(j < 10) {
            ...
            await db.createOrUpdateDocument(`drafts/${week15Data.Card.LeagueId}/scores/${gameweek}/cards`, cardId, scoresDoc, false)
            //console.log(scoresDoc)
            //await utils.sleep(10000)
        }

        if(leagueNum == 1 || leagueNum == 4) {
            if(i == 0) {
                leagueNum++;
            } else {
                if (justFlipped) {
    ...
    for( let i = 0; i < 4; i++) {
        ...
        await db.createOrUpdateDocument('drafts', leagueId, league, true)
        console.log(`Created doc for ${leagueId}`)
    }
}

const transitionToRound2HOF = async () => {
    const oldGameweek = '2023REG-15';
...
(async () => {
    ...
    // const leagueMap = create4PlayoffLeagues()
    // await createRound2PlayoffLeagues(rankedCards, leagueMap)
    //await transitionToRound2HOF()
})()
