...
export default function DraftingPage() {
  ...
  }, [user?.id, user?.walletAddress, isLive]);
  ...
  const handleDraftClick = async (draft: Draft) => {
    ...
    if (draft.specialType && draft.id.startsWith('queue-')) {
      ...
      try {
        ...
        const queueType = parts[1]; // jackpot or hof
        const roundId = parseInt(parts[2]) || 1;
  ...
  const getLiveState = (draft: Draft): LiveState => {
    ...
    if (draft.preSpinStartedAt) {
      ...
      // Special drafts (countdown phase): no pre-spin-countdown, go straight to draft-starting
