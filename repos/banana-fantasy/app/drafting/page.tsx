      ...
      {activeDrafts.length === 0 && (
        <div className="space-y-4">
          ...
          <div
            ...
            style={{ background: 'linear-gradient(180deg, rgba(251,191,36,0.06) 0%, rgba(251,191,36,0.01) 100%)' }}
          >
            <div className="absolute inset-0" style={{ backgroundImage: 'radial-gradient(circle at 50% 0%, rgba(251,191,36,0.08) 0%, transparent 60%)' }} />
            <div className="relative px-6 pt-6 pb-5 flex flex-col items-center">
              <h2 className="text-xl font-bold text-white tracking-tight">Banana Best Ball IV</h2>
              <div className="flex items-center gap-3 mt-4">
                <span className="text-[12px] font-medium text-banana/70 bg-banana/[0.08] px-3 py-1.5 rounded-full">$100K Prize Pool</span>
                <span className="text-[12px] font-medium text-white/50 bg-white/[0.05] px-3 py-1.5 rounded-full">$25K 1st Place</span>
                {((user?.draftPasses || 0) + (user?.freeDrafts || 0)) > 0 ? (
                  <button
                    onClick={handleEnterDraft}
                    className="px-5 py-1.5 bg-banana text-black font-semibold text-[12px] rounded-full hover:brightness-110 active:scale-[0.98] transition-all"
                  >
